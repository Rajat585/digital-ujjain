...
const headings = {
  hi: {
    title: "गैलरी",
    subtitle: "पुरानी सिंहस्थ, मंदिर, घाट और सांस्कृतिक पल, एक झलक में",
  },
  en: {
    title: "Gallery",
    subtitle:
...
const viewMoreLabel = {
  hi: "और देखें ↓",
  en: "View More ↓",
  hinglish: "Aur Dekhein ↓",
};
const viewLessLabel = {
  hi: "कम देखें ↑",
  en: "View Less ↑",
  hinglish: "Kam Dekhein ↑",
};
...
export default function Gallery() {
  ...
  return (
    ...
    >
      ...
      <div
        className="w-full max-w-4xl mb-12 gallery-highlights-swiper"
        style={{
      ...
      >
        ...
        >
          {highlightImages.map((img, i) => (
            <SwiperSlide key={`${img.file}-${i}`}>
              <div className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-ujjain-gold/30">
                <img
                  src={wikiImg(img.file)}
                  alt={img.caption}
                  loading="lazy"
                ...
                <p className="absolute bottom-3 left-3 right-3 text-ujjain-cream text-sm font-medium">
                  {img.caption}
                </p>
              </div>
            </SwiperSlide>
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-10">
        <button
          ...
          className={`px-4 py-1.5 rounded-full text-xs md:text-sm font-semibold border transition ${activeCategory === "all" ? "bg-ujjain-gold text-ujjain-dark border-ujjain-gold" : "text-ujjain-cream/70 border-ujjain-gold/30 hover:border-ujjain-gold"}`}
        >
          {allLabel[lang]}
        </button>
        {categories.map((cat) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs md:text-sm font-semibold border transition ${activeCategory === cat.key ? "bg-ujjain-gold text-ujjain-dark border-ujjain-gold" : "text-ujjain-cream/70 border-ujjain-gold/30 hover:border-ujjain-gold"}`}
          >
            {cat.label}
          </button>
        ))}
