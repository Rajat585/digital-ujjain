...
export default function WhatsAppButton() {
  ...
  return (
    <a
      ...
      style={{
        opacity: mounted ? 1 : 0,
        transform: mounted ? undefined : "scale(0.3) rotate(-30deg)",
        transition: "opacity 650ms cubic-bezier(0.34, 1.56, 0.64, 1) 550ms, transform 650ms cubic-bezier(0.34, 1.56, 0.64, 1) 550ms",
      }}
      className="fixed bottom-24 right-4 md:bottom-6 md:right-6 z-40 flex items-center justify-center w-14 h-14"
    >
      <span className="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-40" />
      <span className="relative flex items-center justify-center w-14 h-14 rounded-full bg-[#25D366] shadow-lg shadow-black/40 transition-transform duration-300 hover:scale-125 hover:rotate-6 active:scale-95">
        <svg
          viewBox="0 0 24 24"
          fill="white"
          className="w-7 h-7"
